...
*/
bool Book::operator== (Book const &right) const{
  if(title_ != right.title_) return false;
  if (author_ != right.author_) return false;
  if (page_count_!= right.page_count_) return false;
  if (is_digital_ != right.is_digital_) return false;
  return true;
}
...
*/
bool Book::operator!= (Book const &right) const{
  if(title_ != right.title_) return true;
  if (author_ != right.author_) return true;
  if (page_count_ != right.page_count_) return true;
  if (is_digital_ != right.is_digital_) return true;
  return false;
}
...
void Book::display() const{
    ...
    std::cout << title_ << " is written by " << author_ << ". Page Count: " << page_count_ << ". " << available << " available digitally.\n";
