...
*/
template<class ItemType>
ArrayBag<ItemType>& ArrayBag<ItemType>::operator+=(const ArrayBag<ItemType> &object){
...
template<class ItemType>
ArrayBag<ItemType>& ArrayBag<ItemType>::operator/=(const ArrayBag<ItemType> &object){
      ArrayBag<ItemType> *temp = new ArrayBag<ItemType>();
      for(int i = 0; i < object.getCurrentSize(); i++){
         if(!contains(object.items_[i])){
            temp->add(object.items_[i]);
         }
      }
      return *this;
} 
